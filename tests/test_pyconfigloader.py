...
from pyconfigloader import Configuration, _dict_merge, ConfigurationException
...
class TestConfigLoader(unittest.TestCase):
    ...
    def test_update_from_yaml_file(self):
        self._test_update_from_file(test_yaml,
                                    self.config.update_from_yaml_file,
                                    test_yaml_output)

    # @skip_if_yaml_not_available
    def test_update_from_yaml_file_obj(self):
        self._test_update_from_file_obj(test_yaml,
                                        self.config.update_from_yaml_file,
                                        test_yaml_output)

    def test_update_from_ini_file(self):
        self._test_update_from_file(test_ini,
                                    self.config.update_from_ini_file,
                                    test_ini_output)

    def test_update_from_ini_file_obj(self):
        self._test_update_from_file_obj(test_ini,
                                        self.config.update_from_ini_file,
                                        test_ini_output)

    def test_update_from_json_file(self):
        self._test_update_from_file(test_json,
                                    self.config.update_from_json_file,
                                    test_json_output)

    def test_update_from_json_file_obj(self):
        self._test_update_from_file_obj(test_json,
                                        self.config.update_from_json_file,
                                        test_json_output)

    def test_update_from_properties_file(self):
        self._test_update_from_file(test_properties,
    ...
    def test_load_config_least_important_files(self):
        with temp_config_file(test_json, '.json') as test_file1_name, \
                temp_config_file(test_yaml, '.yaml') as test_file2_name:
    ...
    def test_load_config_least_and_most_important_files(self):
        with temp_config_file(test_json, '.json') as test_file1_name, \
                temp_config_file(test_yaml, '.yaml') as test_file2_name:
            self.config.load_config(least_important_files=[test_file1_name],
    ...
    def test_load_config_least_and_most_important_files_inversed(self):
        with temp_config_file(test_json, '.json') as test_file1_name, \
                temp_config_file(test_yaml, '.yaml') as test_file2_name:
            self.config.load_config(least_important_files=[test_file2_name],
    ...
    def test_load_config_least_important_dir(self):
        with temp_config_file(test_json, '.json') as test_file_name:
            self.config.load_config(app_name=os.path.splitext(os.path.basename(test_file_name))[0],
    ...
    def test_namespace(self):
        config = Configuration(MY_APP_SETTING1='a',
    ...
    def test_sub_configuration_nominal(self):
        ...
        config.update_from_json_file(io.StringIO(textwrap.dedent("""
        {
            "KEY1": "val1",
            "KEY2": {"SUBKEY": "subval"}
        }
        ...
        expected.update_from_json_file(io.StringIO(textwrap.dedent("""
        {
            "SUBKEY": "subval"
        }
        """)))
    ...
    def test_sub_configuration_error_simple_value(self):
        ...
        """)))
        self.assertRaises(ConfigurationException, config.sub_configuration, 'KEY1')
